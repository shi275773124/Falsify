...
## CI gate boundaries / bypass model
...
- **Truncated reviewer output**: if the model response is cut before the final verdict line, the CLI defaults to `BLOCK`. That is safe but can be noisy; raise the output budget or shorten the draft.
