...
第 1 层互审（docs 03）拦的是**把错数字写漂亮**。它拦不住更难的一类：**事实对、结论错**——用错的工具处理了正确的数据，或者把局部事实当成完整结论，或者"fail-close（静默关闭）——写个 ABORT 文件就算完成"。
...
规则:轮与轮之间不要追加辩论,而是 commit 修复版,让审议者 **独立重新判定**。"我已经认错了"不能软化下一轮的 verdict。修复只有经受一次全新审查才算数。
...
不需要两台主机。**两个不同模型族 + 上下文隔离**就够独立了。审议者可以是对一个不同模型的一次性调用,走任意 OpenAI 兼容端点。审议者的 system prompt 必须写:*找漏洞,不客套,不复述对方*。

做法 + 可直接复制的调用:[`examples/cross-model-rpc.md`](../examples/cross-model-rpc.md)。

## 4. 裁决即文件（verdict-as-file）+ 步骤守卫
...
用 [`templates/step-verify.sh`](../templates/step-verify.sh) 守每一步:fail loud（响亮失败：非零退出 + 告警），绝不 fail silent（静默失败）。
...
前沿模型的程序化外语 / 隐蔽通道（covert channel）研究给 Falsify 的提醒是：另一个模型说"我审过了"还不够。AI 可能在一层人类和 monitor 都不可靠审计的文本/代码结构里工作。

不要新增第四层。把这些检查并入对抗审查：

- **人类可审计性。** verdict 是否落到 raw artifact、可读 diff、命令、fixture、源链接等人能检查的证据？如果只依赖 AI summary，BLOCK。
- **语义 verdict 诱导。** 普通语言是否在没有满足 gate 的情况下推动审议者判 `PASS` / `PASS_WITH_DEBT` / `Known Debt`？例如"not blocking / 证据已足够 / 另一个模型同意 / 只是理论风险 / blocker 改名 debt"。
- **Prompt-only audit theater。** 只写一句"注意 prompt injection"不是防线。优先使用已知模式检查、fixture 或 reproducer。模式库只覆盖已知模式，不能证明 unknown channel 不存在。
...
| 多步自动化 pipeline | 再加裁决即文件 + `step-verify.sh` |

## 待解问题

剩下的漏洞:**AI 反向操纵审议者**——用一个*表面*修复(把东西改个名而不是真修)让审议者觉得"已解决"。审议者是不是也该把自己的断言过一遍 G1–G4?未解。
