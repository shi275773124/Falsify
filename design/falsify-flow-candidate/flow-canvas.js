(() => {
  ...
  document.addEventListener("visibilitychange",sync); addEventListener("resize",()=>{resize();drawStatic();},{passive:true});
  addEventListener("scroll",onScroll,{passive:true}); reduceMotion.addEventListener("change",sync); mobile.addEventListener("change",()=>{resize();sync();});
