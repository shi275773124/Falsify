...
> **Chris improvements (this fork).** Before the Iron Law, every mode also loads
> `chris-improvements.md` from this directory. It adds three rules:
> Evidence Gate (anti-fabrication), Scope Refusal (skip throwaway scripts), and
> Light Mode (middle gear for <50-line changes). Where it conflicts with the
> rules below, the chris-improvements file wins.
